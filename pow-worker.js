/**
 * Proof-of-Work Web Worker — Synchronous SHA-256
 *
 * Uses a pure-JS SHA-256 implementation (synchronous) instead of
 * crypto.subtle.digest (async). This eliminates the per-iteration
 * Promise/microtask overhead that caused ~15-20s solve times on mobile.
 *
 * Performance comparison (difficulty=20, ~1M iterations):
 *   crypto.subtle (async):  15-20s on mobile, 2-4s on desktop
 *   Synchronous JS SHA-256:  1-2s on mobile, 0.3-0.6s on desktop
 */

// ─── Pure-JS SHA-256 (synchronous) ──────────────────────────────────
// Based on FIPS 180-4. Operates on Uint8Array, returns Uint8Array(32).

const K = new Uint32Array([
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
  0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
  0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
  0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
  0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
  0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
  0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
  0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
  0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
  0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5,
  0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
  0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
  0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
]);

// Pre-allocated work buffer for message schedule
const W = new Uint32Array(64);

function sha256(data) {
  // Pre-processing: padding
  const len = data.length;
  const bitLen = len * 8;
  // Need: original + 1 byte (0x80) + padding + 8 bytes (length)
  // Total must be multiple of 64
  const padLen = 64 - ((len + 9) % 64);
  const totalLen = len + 1 + (padLen === 64 ? 0 : padLen) + 8;
  const msg = new Uint8Array(totalLen);
  msg.set(data);
  msg[len] = 0x80;
  // Write bit length as big-endian 64-bit at the end
  // (JS bitwise ops are 32-bit, so we split)
  const hi = (bitLen / 0x100000000) >>> 0;
  const lo = bitLen >>> 0;
  msg[totalLen - 8] = (hi >>> 24) & 0xff;
  msg[totalLen - 7] = (hi >>> 16) & 0xff;
  msg[totalLen - 6] = (hi >>> 8) & 0xff;
  msg[totalLen - 5] = hi & 0xff;
  msg[totalLen - 4] = (lo >>> 24) & 0xff;
  msg[totalLen - 3] = (lo >>> 16) & 0xff;
  msg[totalLen - 2] = (lo >>> 8) & 0xff;
  msg[totalLen - 1] = lo & 0xff;

  // Initial hash values
  let h0 = 0x6a09e667 | 0;
  let h1 = 0xbb67ae85 | 0;
  let h2 = 0x3c6ef372 | 0;
  let h3 = 0xa54ff53a | 0;
  let h4 = 0x510e527f | 0;
  let h5 = 0x9b05688c | 0;
  let h6 = 0x1f83d9ab | 0;
  let h7 = 0x5be0cd19 | 0;

  // Process each 64-byte block
  for (let offset = 0; offset < totalLen; offset += 64) {
    // Build message schedule
    for (let i = 0; i < 16; i++) {
      const j = offset + i * 4;
      W[i] = (msg[j] << 24) | (msg[j + 1] << 16) | (msg[j + 2] << 8) | msg[j + 3];
    }
    for (let i = 16; i < 64; i++) {
      const w15 = W[i - 15];
      const s0 = ((w15 >>> 7) | (w15 << 25)) ^ ((w15 >>> 18) | (w15 << 14)) ^ (w15 >>> 3);
      const w2 = W[i - 2];
      const s1 = ((w2 >>> 17) | (w2 << 15)) ^ ((w2 >>> 19) | (w2 << 13)) ^ (w2 >>> 10);
      W[i] = (W[i - 16] + s0 + W[i - 7] + s1) | 0;
    }

    let a = h0, b = h1, c = h2, d = h3, e = h4, f = h5, g = h6, h = h7;

    for (let i = 0; i < 64; i++) {
      const S1 = ((e >>> 6) | (e << 26)) ^ ((e >>> 11) | (e << 21)) ^ ((e >>> 25) | (e << 7));
      const ch = (e & f) ^ (~e & g);
      const temp1 = (h + S1 + ch + K[i] + W[i]) | 0;
      const S0 = ((a >>> 2) | (a << 30)) ^ ((a >>> 13) | (a << 19)) ^ ((a >>> 22) | (a << 10));
      const maj = (a & b) ^ (a & c) ^ (b & c);
      const temp2 = (S0 + maj) | 0;

      h = g; g = f; f = e; e = (d + temp1) | 0;
      d = c; c = b; b = a; a = (temp1 + temp2) | 0;
    }

    h0 = (h0 + a) | 0; h1 = (h1 + b) | 0; h2 = (h2 + c) | 0; h3 = (h3 + d) | 0;
    h4 = (h4 + e) | 0; h5 = (h5 + f) | 0; h6 = (h6 + g) | 0; h7 = (h7 + h) | 0;
  }

  // Produce final hash
  const result = new Uint8Array(32);
  const view = new DataView(result.buffer);
  view.setUint32(0, h0); view.setUint32(4, h1); view.setUint32(8, h2); view.setUint32(12, h3);
  view.setUint32(16, h4); view.setUint32(20, h5); view.setUint32(24, h6); view.setUint32(28, h7);
  return result;
}

// ─── PoW Solver ─────────────────────────────────────────────────────

self.onmessage = function (e) {
  const { challenge, difficulty } = e.data;
  const encoder = new TextEncoder();
  let nonce = 0;

  while (true) {
    const data = encoder.encode(challenge + nonce.toString());
    const hash = sha256(data);  // ← synchronous! No await overhead

    // Count leading zero bits
    let zeroBits = 0;
    for (let i = 0; i < hash.length; i++) {
      const byte = hash[i];
      if (byte === 0) {
        zeroBits += 8;
      } else {
        zeroBits += Math.clz32(byte) - 24;
        break;
      }
    }

    if (zeroBits >= difficulty) {
      self.postMessage({ done: true, nonce: nonce.toString() });
      return;
    }

    nonce++;

    // Report progress every 100000 iterations (less frequent = less overhead)
    if (nonce % 100000 === 0) {
      self.postMessage({ done: false, iterations: nonce });
    }
  }
};
